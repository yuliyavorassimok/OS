...
HANDLE start_process(wstring command_line) {
	...
	if (!CreateProcess(nullptr, (LPWSTR)command_line.c_str(), nullptr, nullptr,
...
int main() {
	...
	HANDLE write_sem = CreateSemaphore(nullptr, number_of_records, number_of_records, L"write_sem");
	HANDLE read_sem = CreateSemaphore(nullptr, 0, number_of_records, L"read_sem");
	...
	for (int i = 0; i < number_of_senders; ++i) {
		...
		HANDLE event = CreateEvent(nullptr, FALSE, FALSE, (to_wstring(i) + L"ready").c_str());
		events[i] = event;
	...
	while (true) {
		...
		file.close();
		ReleaseMutex(mutex);
		ReleaseSemaphore(write_sem, 1, nullptr);
