...
int main(int argc, char* argv[]) {
	...
	HANDLE write_sem = OpenSemaphore(SEMAPHORE_ALL_ACCESS, FALSE, (LPWSTR)L"write_sem");
	HANDLE read_sem = OpenSemaphore(SEMAPHORE_ALL_ACCESS, FALSE, (LPWSTR)L"read_sem");
	...
	while (true) {
		...
		file.close();
		ReleaseMutex(mutex);
		ReleaseSemaphore(read_sem, 1, nullptr);
